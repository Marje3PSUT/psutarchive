<template>
  <div
    ...
    :style="{
      borderColor: props.item.category![0]
        ? (props.item.category![0].category_id as Category).color
        : undefined,
    }"
  >
    <div class="flex justify-between m-2 px-2 gap-2 items-center">
      <span
        class="text-xs badge bg-transparent border border-base-content border-opacity-40"
      >
        #{{ item.course_id }}
      </span>
    ...
    >
      ...
      <div
        class="card-info flex justify-between items-center text-xs w-full mt-auto"
      >
        <span
...
const categories = ref(
  props.item
    .category!.map((category_course) =>
      locale.value === "en"
        ? (category_course.category_id as Category)!.name_en
        : (category_course.category_id as Category)!.name_ar,
    )
    .join(", "),
);
const resCount = ref(
  props.item.category!.length ? props.item.resource!.length : 0,
);
</script>
