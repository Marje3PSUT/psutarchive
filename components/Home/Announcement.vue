...
<template>
  <div v-if="open" class="announcement-container" :class="{ 'flex-wrap': expanded }">
    <Icon name="ion:speakerphone" size="24" class="min-w-[24px]" />
    <!-- title -->
    <span class="font-bold capitalize cursor-default" @click="expanded = !expanded">
    ...
    <!-- link -->
    <a v-if="data.link" :href="data.link" class="btn btn-circle" target="_blank">
      <Icon name="ion:open-outline" size="24" class="min-w-[24px]" />
    </a>
    <!-- close -->
    <button title="Hide announcement." class="btn btn-circle ms-auto" @click="hide()">
      <Icon name="ion:close" />
    </button>
  </div>
</template>
